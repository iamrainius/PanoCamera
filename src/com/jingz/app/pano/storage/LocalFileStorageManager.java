...
public class LocalFileStorageManager implements StorageManager {
	...
	private void cleanUpOldSessions(long paramLong) {
		File[] arrayOfFile = getSessionBaseDirectory().listFiles(
				new FileFilter() {
					public boolean accept(File paramFile) {
		...
		for (int j = 0; j < i; ++j) {
			...
			if ((localFile.lastModified() >= l - paramLong)
					|| (FileUtil.deleteDirectoryRecursively(localFile)))
				continue;
			Log.w("LightCycle-storage",
					"Could not clean up " + localFile.getAbsolutePath());
		}
	}
	...
	boolean addExistingPanoramaSession(File paramFile, int paramInt,
			float paramFloat) {
		File localFile1 = getThumbnailDirectory();
		if (localFile1 == null)
			Log.e("LightCycle-storage",
					"Could not get the thumbnail directory.");
		File localFile2;
		do {
			return false;
			localFile2 = new File(localFile1, paramFile.getName());
		} while ((!localFile2.exists())
				&& (!LightCycleNative.CreateThumbnailImage(
						paramFile.getAbsolutePath(),
						localFile2.getAbsolutePath(), paramInt, paramFloat)));
	...
	public void addExistingPanoramaSessions(Callback<Void> paramCallback,
			ProgressDialog paramProgressDialog) {
		new InitializeLocalDatabaseTask(this.mContext, this,
				paramProgressDialog, paramCallback).execute(new Void[0]);
	}
	...
	public void addSessionData(LocalSessionStorage paramLocalSessionStorage)
  {
	...
	public void deleteSession(String paramString) {
		LocalSessionEntry localLocalSessionEntry = this.mLocalDatabase
				.getSession(paramString);
		...
		if (localLocalSessionEntry.captureDirectory != null) {
			...
			if ((localFile.isDirectory()) && (localFile.exists())
					&& (!FileUtil.deleteDirectoryRecursively(localFile)))
				Log.e("LightCycle-storage",
						"Unable to delete pano capture directory.");
		}
		if (this.mLocalDatabase.deleteSession(localLocalSessionEntry.id))
			return;
		Log.e("LightCycle-storage",
				"Unable to delete session entry in local database.");
	}
	...
	public LocalSessionStorage getExistingLocalSessionStorage(String paramString) {
		...
		do {
			...
			if ((localLocalSessionStorage.metadataFilePath != null)
					&& (localLocalSessionStorage.metadataFilePath
							.endsWith("metadata.csv")))
				localLocalSessionStorage.metadataFilePath = localLocalSessionStorage.metadataFilePath
						.replace("metadata.csv", "session.meta");
			if (localLocalSessionEntry.stitchedExists)
		...
		localLocalSessionStorage.orientationFilePath = new File(new File(
				localLocalSessionStorage.sessionDir), "orientations.txt")
				.getAbsolutePath();
		return localLocalSessionStorage;
	}

	public LocalSessionStorage getLocalSessionStorage() {
		String str1 = new SimpleDateFormat("yyyyMMdd_HHmmss")
				.format(new Date());
		...
		try {
			...
			localLocalSessionStorage.mosaicFilePath = new File(
					this.mPanoDirectory, str2).getAbsolutePath();
			String str3 = "preview_" + str1 + ".jpg";
			localLocalSessionStorage.previewMosaicFilePath = new File(
					this.mPanoDirectory, str3).getAbsolutePath();
			if (getThumbnailDirectory() == null)
				Log.e("LightCycle-storage",
						"Could not get the thumbnail directory.");
			File localFile2;
			for (localLocalSessionStorage.thumbnailFilePath = "";; localLocalSessionStorage.thumbnailFilePath = localFile2
					.getAbsolutePath()) {
				localLocalSessionStorage.orientationFilePath = new File(
						localFile1, "orientations.txt").getAbsolutePath();
				localLocalSessionStorage.metadataFilePath = new File(
						localFile1, "session.meta").getAbsolutePath();
				return localLocalSessionStorage;
	...
	public File getSessionBaseDirectory() {
		File localFile = new File(this.mContext.getExternalFilesDir(null),
				"panorama_sessions");
		if ((!localFile.exists()) && (!localFile.mkdirs())) {
			Log.e("LightCycle-storage",
					"Sessions root directory could not be created.");
			localFile = null;
		}
		return localFile;
	}
	...
	public ZippableSession getZippableSession(String paramString) {
		return new LocalZippableSession(
				this.mLocalDatabase.getSession(paramString));
	}
	...
	public File retrieveStitchedPanorama(String paramString) {
		return new File(
				this.mLocalDatabase.getSession(paramString).stitchedFile);
	}

	public File retrieveThumbnail(String paramString) {
		return new File(
				this.mLocalDatabase.getSession(paramString).thumbnailFile);
	}
 ...
 * Qualified Name:
 * com.google.android.apps.lightcycle.storage.LocalFileStorageManager JD-Core
 * Version: 0.5.4
 */