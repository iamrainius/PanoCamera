...
public class PanoMetadata {
	...
	private PanoMetadata(int paramInt1, int paramInt2, Calendar paramCalendar1,
			Calendar paramCalendar2, int paramInt3, String paramString,
			boolean paramBoolean1, int paramInt4, int paramInt5, int paramInt6,
			int paramInt7, int paramInt8, int paramInt9, int paramInt10,
			int paramInt11, int paramInt12, int paramInt13,
	...
	private static boolean getBoolean(XMPMeta paramXMPMeta, String paramString)
			throws XMPException {
		if (paramXMPMeta.doesPropertyExist(
				"http://ns.google.com/photos/1.0/panorama/", paramString))
			return paramXMPMeta.getPropertyBoolean(
					"http://ns.google.com/photos/1.0/panorama/", paramString)
					.booleanValue();
		return false;
	}
	...
	private static Calendar getDate(XMPMeta paramXMPMeta, String paramString)
		...
		if (paramXMPMeta.doesPropertyExist(
				"http://ns.google.com/photos/1.0/panorama/", paramString))
			return paramXMPMeta.getPropertyCalendar(
					"http://ns.google.com/photos/1.0/panorama/", paramString);
		return null;
	}
	...
	private static int getInt(XMPMeta paramXMPMeta, String paramString)
		...
		if (paramXMPMeta.doesPropertyExist(
				"http://ns.google.com/photos/1.0/panorama/", paramString))
			return paramXMPMeta.getPropertyInteger(
					"http://ns.google.com/photos/1.0/panorama/", paramString)
					.intValue();
		return 0;
	}
	...
	private static String getString(XMPMeta paramXMPMeta, String paramString)
		...
		if (paramXMPMeta.doesPropertyExist(
				"http://ns.google.com/photos/1.0/panorama/", paramString))
			return paramXMPMeta.getPropertyString(
					"http://ns.google.com/photos/1.0/panorama/", paramString);
		return null;
	}

	private static boolean isNear(double paramDouble1, double paramDouble2,
			double paramDouble3) {
		...
		return parse(new InputStreamFactory(paramString) {
			public InputStream create() {
				try {
					...
					Log.e(PanoMetadata.TAG, "Could not read file: "
							+ this.val$filename, localFileNotFoundException);
				}
				return null;
			}
		});
 ...
 * Qualified Name: com.google.android.apps.lightcycle.util.PanoMetadata JD-Core
 * Version: 0.5.4
 */